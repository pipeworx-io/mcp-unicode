...
`tools/list` at `https://gateway.pipeworx.io/unicode/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/char_info \
  -H 'Content-Type: application/json' \
  -d '{"text":"Hello🌍"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/char_info`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
